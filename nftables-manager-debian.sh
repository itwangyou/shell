...
init_env() {
    ...
    if [ "$SCRIPT_PATH" != "$INSTALL_PATH" ]; then
        ...
        case "$0" in
            /dev/fd/*)
                curl -sL "https://raw.githubusercontent.com/itwangyou/shell/main/nftables-manager-debian.sh" -o "$INSTALL_PATH"
                ;;
            *)
                cp "$0" "$INSTALL_PATH"
                ;;
...
setup_autoload() {
    ...
    if [ ! -f /etc/nftables.d/trans-include.conf ]; then
        ...
        printf '%b\n' "${GREEN}✅ 已配置开机自动加载规则 (systemd nftables.service)${NC}"
    else
        printf '%b\n' "${GREEN}✅ 开机自动加载已配置${NC}"
    fi
}
...
main() {
    ...
    while true; do
        printf '\n%b\n' "${GREEN}========================================${NC}"
        printf '%b\n' "${GREEN}  Debian nftables 中转管理工具 (v1.0)${NC}"
        printf '%b\n' "${GREEN}========================================${NC}"
        printf '%b\n' "  [1] 添加转发线路"
        printf '%b\n' "  [2] 查看当前规则"
        printf '%b\n' "  [3] 删除指定线路"
